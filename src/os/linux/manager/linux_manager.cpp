...
void linux_manager::_start_unit_at_index(size_t in) {
    execute(units[in]->data.exec.start.pre_start, true);
    command_exec_data res{};
    execute(units[in]->data.exec.start.start, false, &res);
    execute(units[in]->data.exec.start.pre_start, true);
...
void linux_manager::_stop_unit_at_index(size_t in) {
    execute(units[in]->data.exec.stop.pre_stop, false);
    execute(units[in]->data.exec.stop.stop, false);
    execute(units[in]->data.exec.stop.post_stop, false);
